...
class SASSANLSTMModel(Model):
    ...
    def _init_model(self, states:np.array, actions:np.array):
        ...
        # initial layers
        input_states = layers.Input(shape=(states.shape[1], states.shape[2]), name='input_states')
        states_features = layers.Masking(mask_value=self._model_settings['padding_value'], name='masking_states')(input_states)

        input_actions = layers.Input(shape=(actions.shape[1], actions.shape[2]), name='input_actions')
        actions_features = layers.Masking(mask_value=self._model_settings['padding_value'], name='masking_actions')(input_actions)

        # Creating Key, Value, Query
        key_layer = layers.Dense(self._model_settings['key_cells'])(actions_features)
        value_layer = layers.Dense(self._model_settings['value_cells'])(actions_features)
        query_layer = layers.Dense(self._model_settings['query_cells'])(states_features)
        ...
        # Model init
        self._model = Mod([input_states, input_actions], classification_layer)
    ...
    def load_checkpoints(self, checkpoint_path:str, x:list):
        ...
        states, actions = self._format_state_actions_features(x)
        self._init_model(states, actions)
    ...
    def fit(self, x_train:list, y_train:list, x_val:list, y_val:list):
        ...
        val_states, val_actions = self._format_state_actions_features(x_val)

        self._init_model(train_states, train_actions)
        self._history = self._model.fit(
            [train_states, train_actions], y_train,
            validation_data=([val_states, val_actions], y_val),
