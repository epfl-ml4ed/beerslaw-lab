...
class PriorSSANModel(Model):
    ...
    by Artaches Ambartsoumian Fred Popowich [https://arxiv.org/pdf/1812.07860.pdf] and include the concatenation of 
    ...
    def __init__(self, settings:dict):
        ...
        self._name = 'prior self-attention sentiment analysis network'
        self._notation = 'pssan'
    ...
    def _get_csvlogger_path(self) -> str:
        csv_path = '../experiments/{}{}/{}/logger/priorssan/'.format(self._experiment_root, self._experiment_name, self._outer_fold)
    ...
    def _get_model_checkpoint_path(self) -> str:
        path = '../experiments/{}{}/{}/logger/priorssan/'.format(self._experiment_root, self._experiment_name, self._outer_fold)
    ...
    def _init_model(self, priors_train:np.array, features_train:np.array):
        ...
        # initial layers
        input_features = layers.Input(shape=(features_train.shape[1], features_train.shape[2]), name='input')
    ...
    def load_checkpoints(self, checkpoint_path:str, x:list):
        ...
        prior_x, features_x = self._format_prior_features(x)
        self._init_model(prior_x, features_x)
    ...
    def fit(self, x_train:list, y_train:list, x_val:list, y_val:list):
        ...
        self._history = self._model.fit(
            [priors_train, features_train], y_train,
    ...
    def predict(self, x:list) -> list:
        ...
        priors_predict, features_predict = self._format_prior_features(x_predict)
        predictions = self._model.predict([priors_predict, features_predict])
    ...
    def predict_proba(self, x:list) -> list:
        x_predict = self._format_features(x)
        priors_predict, features_predict = self._format_prior_features(x_predict)
        probs = self._model.predict([priors_predict, features_predict])
