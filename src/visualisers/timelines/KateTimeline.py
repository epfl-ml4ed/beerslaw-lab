...
class KateTimeline(Timeline):
    ...
    def _plot_width_value(self, sim: Simulation, glyphs, plot):
        """
        Plot the colour of the solution below the wavelength bar
        Args: 
            sim: simualtion to parse
        ...
        values, ts = sim.get_width()
        values = [original_value, *values]
        ts = [0, *ts]
        values.append(values[-1])
        ts.append(sim.get_last_timestamp())

        nvs = []
        nts = []
        for i in range(len(values) - 1):
            nvs = [*nvs, values[i], values[i]]
            nts = [*nts, ts[i], ts[i+1]]
        nvs = [*nvs, values[-1]]
        nts = [*nts, ts[-1]]
        assert len(nvs) == len(nts)

        variable_values = [(c/maximum_value) * (self._settings['timeline']['value_height']) for c in nvs]
        variable_values = [v + y_coord for v in variable_values]
        source = {
            'x': nts,
            'y': variable_values,
            'colour': [self._palette['timeline'][name] for _ in range(len(nts))],
            'label': [name for _ in range(len(nts))]
        }
        glyphs['width_value'] = plot.line(
            x='x', y='y', 
            color=self._palette['timeline'][name], line_alpha=1, line_width=3,
            source=source, legend_label=name)

        # Guides
        # source = {
        #     'x': [sim.get_last_timestamp() / 2],
        #     'y': [1],
        #     'w': [sim.get_last_timestamp()],
        #     'h': [1],
        #     'colour': [self._palette['timeline'][name]]
        # }
        # plot.rect(x='x', y='y', width='w', height='h', fill_color='colour', fill_alpha=0.5, line_alpha=2, source=source, legend_label='width value')


        plot.add_tools(HoverTool(renderers=[glyphs['width_value']], tooltips=[('variable', "@label")]))
        return glyphs, plot

    def _plot_concentration_value(self, sim: Simulation, glyphs, plot):
        """
        Plot the colour of the solution below the wavelength bar
        Args: 
            sim: simualtion to parse
        ...
        values, ts = sim.get_concentration()
        values = [original_value, *values]
        ts = [0, *ts]
        values.append(values[-1])
        ts.append(sim.get_last_timestamp())
        ...
        plot.add_tools(HoverTool(renderers=[glyphs[name]], tooltips=[('variable', "@label")]))
        return glyphs, plot
    ...
    def _plot_actions(self, sim:Simulation, glyphs, plot, plotter: PlotterSequencing):
        ...
        for label in labels_to_plot:
            ...
            y_coords = [self._settings['timeline']['base_coord'] - 1 for _ in range(len(colours))]
            heights = [self._settings['timeline']['action_height'] - 2 for _ in y_coords]
    ...
    def create_timeline(self, sim: Simulation):
        ...
        glyphs, plot = self._plot_width_value(sim, glyphs, plot)
        glyphs, plot = self._plot_concentration_value(sim, glyphs, plot)
