...
class SeriTimeline(Timeline):
    ...
    The first top bar indicates whether the student was measuring something sensitive with the ruler
    The second top bar indicates what dependent variable the student was looking at
    ...
    def __init__(self, settings: dict):
        self._name = 'seritimeline'
        self._notation = 'srtmln'
    ...
    def _plot_wavelength(self, sim: Simulation, glyphs:dict, plot):
        ...
        glyphs['wavelength (nm)'] = plot.rect(x='x', y='y', width='w', height='h', fill_color='colour', line_alpha=0, source=source, legend_label='wavelength (nm)')
        plot.add_tools(HoverTool(renderers=[glyphs['wavelength (nm)']], tooltips=[('wavelength', "@label")]))
        return glyphs, plot
    ...
    def _plot_solutioncolour(self, sim: Simulation, glyphs, plot):
        ...
        glyphs['solution colour'] = plot.rect(x='x', y='y', width='w', height='h', fill_color='colour', line_alpha=0, source=source, legend_label='solution colour')
        plot.add_tools(HoverTool(renderers=[glyphs['solution colour']], tooltips=[('solution', "@label")]))
        return glyphs, plot
    ...
    def _plot_ruler(self, sim:Simulation, glyphs, plot, plotter: PlotterSequencing):
        ...
        y_coord = self._settings['timeline']['base_coord']
        y_coord += self._settings['timeline']['action_height'] / 2 # action bar
        y_coord += self._settings['timeline']['state_height'] # solution bar
        y_coord += self._settings['timeline']['state_height'] # wavelength bar
        y_coord += self._settings['timeline']['state_height'] # variable observed bar
        y_coord += self._settings['timeline']['state_height'] / 2 # Finding the middle y coordinate
    ...
    def _plot_measuring(self, sim:Simulation, glyphs, plot, plotter: PlotterSequencing):
        ...
        for label in np.unique(values):
            glyphs['dependent variable ' + label] = plot.rect(x=[-10], y=self._settings['timeline']['base_coord'], width=[0], height=[0], fill_alpha=0, fill_color=self._palette['timeline'][label], legend_label='dependent variable: ' + label, line_alpha=0)
    ...
    def _frame_timeline(self, glyphs, plot, sim:Simulation):
        under_coord = self._settings['timeline']['base_coord']
        under_coord -= self._settings['timeline']['action_height'] / 2
        upper_coord = self._settings['timeline']['base_coord']
        upper_coord += self._settings['timeline']['action_height'] / 2 # action bar
        upper_coord += self._settings['timeline']['state_height'] # solution bar
        upper_coord += self._settings['timeline']['state_height'] # wavelength bar
        upper_coord += self._settings['timeline']['state_height'] # Finding the middle y coordinate
        upper_coord += self._settings['timeline']['state_height'] 
        height = self._settings['timeline']['state_height']
        height += self._settings['timeline']['state_height']
        height +=self._settings['timeline']['action_height']
        height += self._settings['timeline']['state_height']
        height += self._settings['timeline']['state_height']
        middle_coord = (upper_coord - under_coord) / 2
        middle_coord = under_coord + middle_coord

        width = sim.get_last_timestamp()

        source = ColumnDataSource(dict(
            x=[sim.get_last_timestamp()/2], y=[middle_coord],
            w=[width], h=[height],
            color=['white']
            ))
